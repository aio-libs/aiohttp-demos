...
class MessageAdded(graphene.ObjectType):
    ...
    id = graphene.Int(description="The id of message")
    body = graphene.String(description="The text of message")
    owner = graphene.Field(User, description="The owner of current message")
...
class StartTyping(graphene.ObjectType):
    ...
    user = graphene.Field(User, description="The user why are typing right now")
...
class MessageSubscription(graphene.ObjectType):
    ...
    typing_start = graphene.Field(StartTyping, room_id=graphene.Argument(graphene.Int))
    message_added = graphene.Field(
        MessageAdded, room_id=graphene.Argument(graphene.Int)
    )
